  ...
  return (
    <nav className="h-16 w-full flex items-center justify-between px-8">
      <div className="flex items-center gap-10">
        ...
        <div
          className={`hidden sm:flex items-center bg-transparent rounded-md px-3 py-1.5 border ${
            mode === "dark"
              ? "border-zinc-700 shadow-zinc-800"
              : "shadow-xs shadow-gray-300 border-gray-200"
          }`}
        >
          <Search
            size={18}
            className={mode === "dark" ? "text-zinc-400" : "text-gray-500"}
          />
          <input
            type="text"
      ...
      <ul
        className={`hidden md:flex items-center gap-6 ${
          mode === "dark" ? "text-zinc-300" : "text-gray-500"
        }`}
      >
        <li
